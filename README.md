...
BLF `SYSTEMTIME` headers and Vector ASC header dates do not include a reliable timezone offset, and real BLF files may contain either UTC calendar values or local wall-clock values depending on the logger. The app keeps the stored calendar values on a UTC baseline, then applies the timezone selected by `采集时区` as a direct display and export offset. The app selects the detected server/browser local timezone by default, falling back to a timezone with the same UTC offset when the operating system only exposes a platform-specific name. The selector is populated from the browser's IANA timezone database and shows timezone offsets with representative cities, for example `UTC`, `Asia/Shanghai`, `Europe/Berlin`, or `America/New_York`.

Changing `采集时区` only shifts and redraws the existing absolute-time axis; it does not decode the log again. CSV export uses the timezone currently selected at export time. For a BLF whose stored values are already local wall-clock time, select `UTC` to leave those values unshifted; for a BLF whose stored values are UTC, select the desired local timezone such as `Asia/Shanghai`. On Windows, `tzdata` is included in `requirements.txt` so Python can also resolve IANA timezone names when exporting.
